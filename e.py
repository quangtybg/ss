import random, json, os, time, multiprocessing
from typing import List, Tuple

SIZE = 15
WIN_LEN = 5
CACHE_FILE = "gomoku_cache.json"
CACHE_LOCK = multiprocessing.Lock()   # khóa khi ghi cache
...
# -------------------------------------------------------
class Bot:
    def __init__(self, player: str, shared_cache):
        self.player = player
        self.cache = shared_cache  # cache dùng chung giữa các tiến trình
    ...
    def choose_move(self, board) -> Tuple[int,int]:
        ...
        for (r,c) in candidates:
            ...
            if self.is_winner(board, me):
                board[r][c] = " "
                with CACHE_LOCK:
                    self.cache[key] = (r,c); save_cache(dict(self.cache))
                return (r,c)
            board[r][c] = " "
...
# API ngoài
# -------------------------------------------------------
def get_move(board: List[List[str]], current_player: str, shared_cache) -> Tuple[int,int]:
    bot = Bot(current_player, shared_cache)
    return bot.choose_move(board)

# -------------------------------------------------------
# Train đa tiến trình
# -------------------------------------------------------
def play_game(game_id: int, shared_cache):
    board = [[" "]*SIZE for _ in range(SIZE)]
    botX, botO = Bot("X", shared_cache), Bot("O", shared_cache)
    player = "X"
    moves = 0
    while moves < SIZE*SIZE:
        ...
        moves += 1
    print(f"Game {game_id} done, moves={moves}")

def train_bots_multiprocess(n_games=100, n_processes=4):
    manager = multiprocessing.Manager()
    shared_cache = manager.dict(load_cache())  # cache chia sẻ giữa tiến trình

    with multiprocessing.Pool(processes=n_processes) as pool:
        pool.starmap(play_game, [(i+1, shared_cache) for i in range(n_games)])

    # lưu cache cuối cùng
    save_cache(dict(shared_cache))
    print("Training finished (multi-process), cache updated.")

# Example usage:
# if __name__ == "__main__":
#     train_bots_multiprocess(n_games=50, n_processes=4)
