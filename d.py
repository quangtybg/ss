...
# -------------------------------------------------------
# Train đa tiến trình + thống kê kết quả
# -------------------------------------------------------
...
def train_bots_multiprocess(n_games=100, n_processes=4):
    ...
    with multiprocessing.Pool(processes=n_processes) as pool:
        results = pool.starmap(play_game, [(i+1, shared_cache) for i in range(n_games)])
    ...
    for r in results:
        stats[r] += 1
    total = n_games
    print("Training finished (multi-process). Stats:")
    print(f"  X wins: {stats['X']} ({stats['X']/total:.1%})")
    print(f"  O wins: {stats['O']} ({stats['O']/total:.1%})")
    print(f"  Draws : {stats['D']} ({stats['D']/total:.1%})")
