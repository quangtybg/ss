...
# Pattern evaluation for scoring
def pattern_score(line: List[str], player: str) -> int:
    ...
    if opp*4 in s and s.count(" ")>=1: score -= 10000
    # Open 3
    if player*3 in s and s.count(" ")>=2: score += 1000
    if opp*3 in s and s.count(" ")>=2: score -= 20000  # extreme danger
    # Open 2
...
def find_immediate(board: List[List[str]], player: str) -> Optional[Tuple[int,int]]:
    ...
    for r,c in moves:
        ...
        if evaluate(board, opp) >= 100000:
            ...
            return (r,c)
        board[r][c] = " "
    # Check block opponent open 3/4 (threat)
    for r,c in moves:
        board[r][c] = opp
