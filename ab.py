...
# --- Threat Space Search + Alpha-Beta ---
def alphabeta_tss(board, depth, alpha, beta, maximizing, player, start_time, time_limit=0.9):
    ...
    # Threat Space Search: ưu tiên moves tạo threat
    threat_moves = []
    for r,c in moves:
        board[r][c] = player
...
# --- Iterative Deepening + Opening Book ---
...
# --- Self-play học opening book ---
def self_play(num_games=50):
    global opening_book
    for g in range(num_games):
        ...
        while True:
            ...
            move = None
            if board_key in opening_book:
                move = tuple(opening_book[board_key])
            else:
                move = get_move(deepcopy(board), current_player)
            r,c = move
            board[r][c] = current_player
            moves_history.append((board_key, move, current_player))
    ...
    print(f"Self-play done, opening book size: {len(opening_book)}")
...
if __name__ == "__main__":
    ...
    print("X đánh:", move)
    print("Bắt đầu self-play học opening book...")
    self_play(50)
